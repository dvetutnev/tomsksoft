...
[Boost.SML](https://boost-ext.github.io/sml/index.html) удивительная вещь. Очень редко когда с новой библиотекой все получается *сразу*, вот с ней очень даже. А описание таблицы состоянийпрямо в нотации **UML** заслуживает отдельного лайка. Вот что дает перегрузка операторов, можно без всяких парсеров реализовать нужный синтаксис.

Статитеский полиморфизм это удобно. Классы **Session** и **Writer** писались и отлаживались на mock-объектах (таймер\сокет\файл), с реальными классами из **uvw** оно заработало сразу. Хотя казалосьбы mock-ки и рабочие классы даже общей базы не имеют. Но документацию на мокируемые объекты нужно читать внимательно и не стесняться для mock-а написать тест.
